...
function activate(context, platformContext, cache, configuration, secrets, store, envStore, prefs, signUpWebView, auditView, auditContext) {
    ...
    const getReportView = async (uri) => {
        ...
        if (reportViews[viewId] === undefined) {
            reportViews[viewId] = new report_view_1.ScanReportWebView(`Scan report ${alias}`, context.extensionPath, cache, configuration, secrets, store, envStore, prefs);
        }
    ...
    activateLens(configuration.get("codeLens"));
    (0, commands_1.default)(cache, platformContext, store, configuration, secrets, getScanView, getExistingReportView, signUpWebView);
